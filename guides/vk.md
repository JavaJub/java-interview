...
> **ВНИМАНИЕ.** По отзывам кандидатов VK, алгоритмы и структуры данных — главный фильтр. Хэш-таблицы, деревья, очереди с приоритетом спрашивают даже у Middle. SQL обычно проходит спокойнее, поэтому алгоритмы стоит тренировать отдельно.

### Финальный чек-лист

| Блок | Готов, если можешь... |
| --- | --- |
| Java Core | Объяснить equals/hashCode, Integer cache, Virtual Threads, Records |
| Коллекции | Рассказать HashMap treeify/resize, ConcurrentHashMap Java 8, PriorityQueue |
| Многопоточность | Сравнить volatile, synchronized, CAS, ThreadPoolExecutor, Virtual Threads pinning |
| Spring | Объяснить @Transactional proxy, self-call, prototype в singleton, Spring Boot 3 |
| SQL | Написать JOIN, GROUP BY, составной индекс, оконные функции, EXPLAIN ANALYZE |
| Kafka | Объяснить consumer group, at-least-once, идемпотентность, exactly-once |
| Сети | Сравнить L3/L4/L7 балансировку, HAProxy, VRRP, health checks |
| Docker/K8s | Собрать multi-stage Dockerfile и объяснить Pod/Deployment/Service/probes |
| System Design | Спроектировать load balancer или rate limiter с rough capacity estimate |
| Live-coding | Решить 3 задачи за час: строки, дубликаты, code review |
